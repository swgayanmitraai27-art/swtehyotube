    ...
    try {
      ...
      const res = await fetch('/api/ai/generate-reply', {
        ...
        body: JSON.stringify({
          ...
          customInstructions: oneTimePrompt.trim() || undefined,
        }),
      });
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-zinc-400 flex items-center gap-1.5">
              ...
              Google Gemma 4 31B Suggestions:
            </span>
          </div>

          {/* Quick Custom Instruction for this comment */}
          <div className="flex gap-2">
            <input
              type="text"
              value={oneTimePrompt}
              onChange={(e) => setOneTimePrompt(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
                  handleRegenerate();
              ...
              placeholder="🪄 Add custom instruction (e.g. Tell him batch starts Monday at 6 PM)..."
              className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-1.5 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-rose-500"
            />
            <button
              type="button"
              onClick={handleRegenerate}
              disabled={isRegenerating}
              className="px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold flex items-center gap-1.5 transition-colors disabled:opacity-50 shrink-0"
            >
              <RefreshCw className={`w-3.5 h-3.5 text-rose-400 ${isRegenerating ? 'animate-spin' : ''}`} />
              {isRegenerating ? 'Thinking...' : 'Apply & Regenerate'}
            </button>
          </div>
